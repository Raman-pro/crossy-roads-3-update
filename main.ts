...
game.onUpdateInterval(500, function () {
    if (Math.percentChance(50)) {
        if (tiles.getTilesByType(assets.tile`water_right`).length > 0) {
            sprite_log = sprites.create([assets.image`log`, assets.image`boat_right`]._pickRandom(), SpriteKind.Log)
        ...
        if (tiles.getTilesByType(assets.tile`water_left`).length > 0) {
            sprite_log = sprites.create([assets.image`log`, assets.image`boat_left`]._pickRandom(), SpriteKind.Log)
